...
namespace InmobiliariaAlcaraz.Controllers
{
    public class UsuarioController : Controller
    {
        ...
        [Authorize]
        [ValidateAntiForgeryToken]
        public ActionResult Perfil(Usuario u, IFormFile AvatarFile, bool EliminarAvatar = false)
        {
            try
            {
        ...
        public async Task<IActionResult> Login(LoginView login, string returnUrl)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        Console.WriteLine($"User not found: {login.Usuario}");
                    ...
                    Console.WriteLine($"User ID being set in claims: {usuario.IdUsuario}");
                    ...
                    {
                         return usuario.Rol == (int)enRoles.Administrador 
                            ? RedirectToAction("Index", "Usuario") 
                            : RedirectToAction("Index", "Propietario");
                        }
                }
